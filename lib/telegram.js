...
const getAdminTargets = async () => {
  ...
  const settingsDoc = await db
    .collection('settings')
    .findOne({ name: 'telegramNotifyUsernames' });
...
const isTelegramAdminRequest = async (ctx) => {
  const chatIdValue = String(ctx?.chat?.id || '').trim();
  const username = normalizeUsername(ctx?.from?.username || '');

  if (username && ALWAYS_ALLOWED_ADMIN_USERNAMES.has(username)) {
    return true;
  }
  ...
  if (username && allowedHandles.has(username)) {
    return true;
  }
...
const setupBot = (bot) => {
  ...
  bot.command('status', async (ctx) => {
    let allowed = false;
    try {
      allowed = await isTelegramAdminRequest(ctx);
    } catch (error) {
      await ctx.reply('Unable to verify admin access right now.');
      return;
    }

    if (!allowed) {
      await ctx.reply(
        'Unauthorized. This command is restricted to configured admin Telegram accounts.'
      );
      return;
    }
    ...
    try {
      const [me, webhook] = await Promise.all([
        ctx.api.getMe(),
        ctx.api.getWebhookInfo(),
      ]);

      const lines = [
        '*Telegram Status*',
        `Bot: ${me.username ? `@${me.username}` : me.id}`,
      ...
      if (webhook?.last_error_date) {
        lines.push(
          `Last error date: ${new Date(
            webhook.last_error_date * 1000
          ).toISOString()}`
        );
      }
      ...
      await ctx.reply('Unable to reach Telegram API right now.');
    }
  });
  ...
  bot.callbackQuery(new RegExp(`^${ORDER_ACTION_PREFIX}\\|`), async (ctx) => {
    ...
    try {
      ...
      await ctx.answerCallbackQuery({
        text: 'Unable to verify admin access right now.',
        show_alert: true,
      });
      return;
    }
